...
public class GrapplingGun : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        ballRigidbody.gravityScale = startGravity;
    }
    ...
    private void Update()
    {
        ...
        if (grappleRope.enabled)
        {
            GameManager.Singleton.player.GetComponent<PlayerMovement>().isGrappled = true;
        }
        else
        {
            GameManager.Singleton.player.GetComponent<PlayerMovement>().isGrappled = false;
        }
    }
    ...
    {
        if (Physics2D.Raycast(firePoint.position, Mouse_FirePoint_DistanceVector.normalized, Mathf.Infinity, grappable))
        {
            ...
            if ((hit)&&((Vector2.Distance(hit.point, firePoint.position) <= maxDistance) || !hasMaxDistance))
            {
